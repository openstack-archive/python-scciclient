...
class ELCMTestCase(testtools.TestCase):
    ...
    @mock.patch.object(elcm, 'elcm_profile_set')
    @mock.patch.object(elcm, '_session_monitoring')
    ...
    @mock.patch.object(elcm, 'elcm_profile_get')
    @mock.patch.object(elcm, 'create_raid_adapter')
    def test_get_raid_config_with_logical_drives(
        ...
        hdd_online_expect = ['0', '1', '4', '5']
        logical_drive_online_expect = [0]

        result = elcm.get_raid_configuration(irmc_info=self.irmc_info)
        self.assertEqual(result[1], hdd_online_expect)
        self.assertEqual(result[2], logical_drive_online_expect)
    ...
    def test_delete_raid_adapter_with_logical_drive_slot_is_none(
        self, elcm_profile_delete_mock, _session_monitoring_mock,
        elcm_profile_set_mock, raid_info_mock):
        raid_info_mock.return_value = [{
            'Server': {
                'HWConfigurationIrmc': {
                    'Adapters': {
