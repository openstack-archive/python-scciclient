...
class SCCITestCase(testtools.TestCase):
    ...
    @mock.patch.object(ipmi, 'get_gpu_fpgas')
    ...
    def test_get_capabilities_properties(self,
        ...
        gpu_fpgas_mock.return_value = (1, 0)

        expected = {'cpu_fpgas': 0,
        ...
        result = scci.get_capabilities_properties(
            self.irmc_info, snmp, CAPABILITIES_PROPERTIES, gpu_ids, fpga_ids)

        self.assertEqual(expected, result)
        tpm_mock.assert_called_once_with(self.irmc_info)
        bios_mock.assert_called_once_with(snmp.SNMPClient())
        irmc_mock.assert_called_once_with(snmp.SNMPClient())
        server_mock.assert_called_once_with(snmp.SNMPClient())
        gpu_fpgas_mock.assert_called_once_with(self.irmc_info,
                                               gpu_ids,
                                               fpga_ids)
