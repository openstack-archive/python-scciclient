...
def get_raid_adapter_profile(irmc_info):
    ...
    :returns: raid_adapter: get latest raid adapter information
              existing_logical_drives: get logical drive on server
    """
    ...
    raid_adapter_profile = elcm_profile_get(irmc_info, PROFILE_RAID_CONFIG)
    raid_adapter = raid_adapter_profile['Server']['HWConfigurationIrmc'][
        'Adapters']['RAIDAdapter'][0]
    logical_drives = raid_adapter.get('LogicalDrives')

    if logical_drives is not None:
        for drive in logical_drives['LogicalDrive']:
            existing_logical_drives.append(drive['@Number'])

    return raid_adapter_profile, existing_logical_drives
...
def create_raid_configuration(irmc_info, target_raid_config):
    ...
    :returns: raid_input: Input information which used to create raid
              configuration.
              result: a dict with following values:
                {
                    'raid_config': <data of raid adapter profile>,
                    'warning': <warning message if there is>
                }
    ...
    raid_input = _update_raid_input_data(target_raid_config,
                                         raid_adapter_profile)
...
def delete_raid_configuration(irmc_info, logical_drive_slot=None):
    ...
    :returns: raid_adapter_profile: latest raid information in Bare metal
    """
    ...
    raid_adapter_profile['Server']['HWConfigurationIrmc'].update({
        '@Processing': 'execute'})
    logical_drive = raid_adapter_profile['Server']['HWConfigurationIrmc'][
