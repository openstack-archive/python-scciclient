...
def _update_raid_input_data(target_raid_config, raid_input, size_gb):
    """Process raid input data.

    :param target_raid_config: node raid info
    :param raid_input: raid information for creating via eLCM
    :param size_gb: default is MAX, list of size of logical drive.
    ...
    array_info = raid_input['Server']['HWConfigurationIrmc']['Adapters'][
        'RAIDAdapter'][0]
    # Set max value for BGI, MDC and Rebuild rates
    array_info['BGIRate'] = array_info['MDCRate'] = \
        array_info['RebuildRate'] = 100
    ...
    for i, logical_disk in enumerate(logical_disk_list):
        ...
        # Allow auto create along with raid 10, raid 50 and raid 60
...
def create_raid_configuration(irmc_info, target_raid_config):
    ...
    size_gb = []
    for size in target_raid_config['logical_disks']:
        size_gb.append(size['size_gb'])
    ...
    raid_input = _update_raid_input_data(
        target_raid_config, raid_adapter, size_gb)
