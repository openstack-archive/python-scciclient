...
def _get_raid_input_data(target_raid_config, raid_input):
    ...
    while i < len(physical_disks):
        ...
        # TODO(trungnv): Will allow configure raid 10 and raid 50 along
        # with specific physical driver.
        if not physical_disks[i] or hard_disk_list[i]['raid_level'] == '10' \
                or hard_disk_list[i]['raid_level'] == '50':
            arrays = {
                "@Number": i,
            ...
            array_info['LogicalDrives']['LogicalDrive'][i].update({
                "@Number": i})
            # Nothing to do with this array format
            ...
            elements_level = hard_disk_list[i]['raid_level']
            lo_drive.update({'RaidLevel': elements_level})
            lo_drive['ArrayRefs']['ArrayRef'].append({"@Number": i})

            for element in hard_disk_list[i]['physical_disks']:
...
def get_raid_configuration(irmc_info):
    ...
    :returns: raid_adapter: get latest raid adapter information
              hdd_online: get HDD online on physical server
    ...
    # created raid cfg.
    _create_raid_adapter(irmc_info)

    raid_adapter = elcm_profile_get(irmc_info, PROFILE_RAID_CONFIG)
    phy_disks = raid_adapter['Server']['HWConfigurationIrmc']['Adapters'][
        'RAIDAdapter'][0]
    physical_disk = phy_disks['PhysicalDisks']['PhysicalDisk']
    logical_drives = phy_disks['LogicalDrives']
    for disk in physical_disk:
        # Get hdd online and status of them.
        hdd_drives_online.append({disk['@Number']: disk['PDStatus']})

    if logical_drives is not None:
        for drive in logical_drives['LogicalDrive']:
            logical_drives_online.append(drive['@Number'])
        return raid_adapter, hdd_drives_online, logical_drives_online

    return raid_adapter, hdd_drives_online
...
def delete_raid_configuration(irmc_info, logical_drive_slot=None):
    ...
    raid_configuration = get_raid_configuration(irmc_info)
    hw_cfg_irmc = raid_configuration[0]['Server']['HWConfigurationIrmc']
    hw_cfg_irmc.update({'@Processing': 'execute'})
    logical_drive = hw_cfg_irmc['Adapters']['RAIDAdapter'][0]['LogicalDrives'][
        'LogicalDrive']

    logical_drive_online = raid_configuration[2]
    ...
    session = elcm_profile_set(irmc_info, raid_configuration[0])
