...
def set_bios_configuration(irmc_info, settings):
    ...
    bios_list = [bios_list for bios_list in bios_config_data.items()]
    for items in bios_list:
        if not re.match('^@', items[0]) and isinstance(items[1], dict):
            bios_config_data.pop(items[0], None)
