...
def get_raid_bgi_status(report):
    ...
    """
    bgi_status = []
    raid_dir = "./Software/ServerView/ServerViewRaid"

    if not report.find(raid_dir):
        raise SCCIInvalidInputError(
            "ServerView RAID not available in Bare metal Server")
    if not report.find(raid_dir + "/amEMSV/System/Adapter/LogicalDrive"):
        raise SCCIInvalidInputError(
            "RAID configuration not configure in Bare metal Server yet")

    logical_drives = report.findall(raid_dir +
