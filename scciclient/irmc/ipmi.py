...
def get_tpm_status(d_info):
    ...
    out = ' '.join('{:02x}'.format(x) for x in response['data']).upper()
...
def _pci_seq(ipmicmd):
    ...
    for i in range(1, 0xff + 1):
        res = send_raw_command(ipmicmd, GET_GPU_FPGA % hex(i))
        yield i, res
    ...
    try:
        def _pci_count(accm, v):
            ...
            if pci_id in pci_device_ids[0]:
                accm = accm[0] + 1, accm[1]
            if pci_id in pci_device_ids[1]:
                accm = accm[0], accm[1] + 1
            return accm

        gpu_count, fpga_count = functools.reduce(_pci_count, response, (0, 0))
    except (TypeError, IndexError) as e:
        raise InvalidParameterValue("Parameter Value is invalid. %s" % e)
