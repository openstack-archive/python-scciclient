...
def exec_ipmitool(d_info, command, temp_dir):
    ...
    with _make_password_file(password or '\0', temp_dir) as pw_file:
        args.append('-f')
        args.append(pw_file)
...
def _make_password_file(password, tempdir):
    ...
    try:
        ...
        if f is not None:
            f.close()
        raise PasswordFileFailedToCreate("Failed to create the password"
                                         " file. %s" % exc)
...
def get_tpm_status(d_info, temp_dir):
    ...
    out = exec_ipmitool(d_info, cmd, temp_dir)
...
def get_gpu_fpgas(d_info, gpu_ids, fpga_ids, temp_dir):
    ...
    if gpu_ids and fpga_ids:
        ...
        while True:
            cmd = ("raw 0x2E 0xF1 0x80 0x28 0x00 0x1A " +
                   "0x{:02x}".format(record_id) + " 0x00")
