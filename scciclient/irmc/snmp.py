...
class SNMPIRMCFirmwareFailure(Exception):
...
class SNMPBIOSFirmwareFailure(Exception):
...
class SNMPServerModelFailure(Exception):
...
def get_irmc_firmware_version(snmp_client):
    ...
    try:
        ...
        raise SNMPIRMCFirmwareFailure(
            "SNMP operation '%(operation)s' failed: %(error)s" %
            {'operation': "GET IRMC FIRMWARE VERSION", 'error': e})
...
def get_bios_firmware_version(snmp_client):
    ...
    try:
        ...
        raise SNMPBIOSFirmwareFailure(
            "SNMP operation '%(operation)s' failed: %(error)s" %
            {'operation': "GET BIOS FIRMWARE VERSION", 'error': e})
...
def get_server_model(snmp_client):
    ...
    try:
        ...
        raise SNMPServerModelFailure(
            "SNMP operation '%(operation)s' failed: %(error)s" %
            {'operation': "GET SERVER MODEL", 'error': e})
...
class SNMPClient(object):
    ...
    def get(self, oid):
        ...
        if error_indication:
            ...
            raise SNMPFailure(
                "SNMP operation '%(operation)s' failed: %(error)s" %
                {'operation': "GET", 'error': error_indication})
        ...
        if error_status:
            ...
            raise SNMPFailure(
                ...
                {'operation': "GET", 'error': error_status.prettyPrint(),
                 'index':
                     error_index and var_binds[int(error_index) - 1][0]
                     or '?'})
    ...
    def get_next(self, oid):
        ...
        if error_indication:
            ...
            raise SNMPFailure(
                "SNMP operation '%(operation)s' failed: %(error)s" %
                {'operation': "GET_NEXT", 'error': error_indication})
    ...
    def set(self, oid, value):
        ...
        if error_indication:
            ...
            raise SNMPFailure(
                "SNMP operation '%(operation)s' failed: %(error)s" %
                {'operation': "SET", 'error': error_indication})
